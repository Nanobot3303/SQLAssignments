...
SELECT CAST(empID AS char(2)) AS ID#, CONVERT(char(20), Name) AS FullName, CAST(Title AS char(20)) AS Position 
FROM dbo.EMPLOYEE
ORDER by Title ASC;
...
SELECT TOP 3 CAST(empID AS char(2)) AS ID#, CONVERT(char(20), Name) AS FullName
FROM dbo.EMPLOYEE
ORDER by empID asc
GO
...
SELECT TOP  3 WITH TIES PartNbr, Cost FROM dbo.PART
ORDER by Cost ASC
--4 vlues are returned because "with ties" does not stop after the requested number of rows. The quesry will also give you
--all the ties with the last row based on cost.
GO
...
SELECT CONVERT(char(20), Quantity) AS CHARQTY, LineNbr
FROM dbo.PO_LINE_ITEM
ORDER by CHARQTY ASC
--it's ordering the column by the number of characters first then by sequential numbers; then it starts the sequence over. That's why you have 10 then 100....then it jumps back to 12 to 14 then 144....then it jumps back to 15 to 150.  
GO
...
SELECT Name, StreetAddress, (city +' ' + StateProvince + ' ' + ZipCode) AS 'City/State/ZIP'
	FROM dbo.CUSTOMER
...
SELECT FK_RepairNbr, LineNbr, CAST((ExtendedPrice + [Labor Charge]) AS float) AS TotalCharge
...
SELECT RepairNbr, FK_StoveNbr, [Description], CONVERT(VARCHAR(8), RepairDt,1)
...
SELECT STR(InvoiceNbr) AS [Invoice Number], '$' + CONVERT(varchar, TotalPrice) AS [Total Price], STR(FK_EmpID) AS [Employee ID] FROM dbo.INVOICE
...
SELECT TOP 1 RepairNbr, Description, Cost from dbo.STOVE_REPAIR
ORDER by Cost DESC
GO
